...
class _Rs3Pb2AliasFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(
        ...
        # Prefer a single shared module object under both names.
...
class _AliasLoader(importlib.abc.Loader):
    ...
    def create_module(self, spec: importlib.machinery.ModuleSpec) -> object | None:
        # Load/return the package-qualified module so protobuf descriptors
        # are registered once.
...
def ensure_rs3_protobuf_imports() -> None:
    ...
    # When collect-all extracted generatedFiles onto disk, mirror RS3's own
    # path hack so bare imports also resolve via the filesystem.
    try:
